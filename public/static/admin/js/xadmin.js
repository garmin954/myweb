  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.add_lay_tab = function(title,url,id) {
		element.tabAdd('xbs_tab', {
	       title: title
	        ,content: '<iframe id="'+id+'" tab-id="'+id+'" frameborder="0" src="'+url+'" scrolling="yes" class="x-iframe"></iframe>'
	        ,id: id
	    })
	}
	/**
...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($('.left-nav').css('width')=='60px'){
          $('.left-nav').animate({width: '220px'}, 300);
          $('.page-content').animate({left: '220px'}, 300);
    ...
    $('.container .left_open i').click(function(event) {
        if($('.left-nav').css('width')=='220px'){
            ...
            $('.left_open').animate({marginLeft: '75px'});
            $('.left-nav').animate({width: '60px'}, 300);
            $('.left-nav cite,.left-nav .nav_right').hide();
            $('.page-content').animate({left: '60px'}, 300);
            ...
            $('.left-nav i').css('font-size','14px');
            $('.left-nav cite,.left-nav .nav_right').show();
            $('.left_open').animate({marginLeft: '235px'}, 300);
            if($(window).width()<768){
                $('.page-content-bg').show();
            }
        }
...
function showChangeBackground() {
    var width = $("#showRight").css('width');
    console.log(width);
    if(width == '0px'){
        $("#showRight").animate({width:"300px"})
        $(".show_box").animate({width:"100%"})
    }else if(width == '300px'){
        $("#showRight").animate({width:"0px"})
        $(".show_box").animate({width:"0%"})
    }
...
function closeBackground() {
    ...
    layui.use(['colorpicker', 'jquery'], function(){
        ...
        colorpicker.render({
            ...
            ,format:'rgb'
            ,alpha:true
            ,colors: ['#009688','#5FB878','#393D49','#1E9FFF','#FFB800','#FF5722','#01AAED','#2F4056']
            ,done: function(color){

                $("#topMenu").parent().parent().attr('style','background:'+color)
                $("#topMenu").siblings().attr('style','color:white')
                $(".left-nav").attr('style','background:'+color)
                setCookie('left_bg', color );
        ...
        colorpicker.render({
            ...
            ,colors: ['#009688','#5FB878','#393D49','#1E9FFF','#FFB800','#FF5722','#01AAED','#2F4056']
            ,done: function(color){
                $("#leftMenu").parent().parent().attr('style','background:'+color)
                $("#leftMenu").siblings().attr('style','color:white')
                $(".left-nav a.active").attr('style','background:'+color)
                setCookie('left_action_bg', color, 99 );
    ...
    $("#refresh").on("click",function(){  //此处添加禁止连续点击刷新一是为了降低服务器压力，另外一个就是为了防止超快点击造成chrome本身的一些js文件的报错(不过貌似这个问题还是存在，不过概率小了很多)
        if($(this).hasClass("refreshThis")){
            ...
            $(".page-content .layui-tab-item.layui-show").find("iframe")[0].contentWindow.location.reload(true);
            setTimeout(function(){
                $("#refresh").addClass("refreshThis");
            },2000)
        }else{
            layer.msg("刷新不要太快了！", {icon:3,anim:6,shade: 0.5});
        }
    })
});
//写cookies
